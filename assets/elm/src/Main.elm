...
import Types.Dropdowns as DropdownType exposing (DropdownItem, DropdownMsg, DropdownNavbarLink, OpenDropdown)
import Types.Page as Page exposing (Page)
import Views.Footer as Footer
import Views.Header as Header exposing (activePageFrom, navDropdownConfig, navDropdownContext, navLinks, viewNavBarLinks)
            ...
            let
                page =
                    case model.session.player of
                        Just player ->
                            Page.Room (Room.initialModel player roomTitle roomType envConfig)
...
frame model isLoading page children =
    ...
    div [ class "page-frame" ]
        ...
        , children
        , if activePage == Helpers.Home then
            Footer.view
          else
            text ""
        ]
    ...
    case ( msg, page ) of
            ...
            let
                ...
                channels =
                    case player of
                        ...
                        Just player ->
                            [ notificationsFor <| usernameToString player.username ]
            in
        ...
        ( FBLogin result, _ ) ->
            let
                cmd =
                    case result of
                        Ok fbData ->
                            Http.send SentLogin (Request.Player.facebookLogin (Facebook.encode fbData) model.apiUrl)
        ...
        ( HeaderMsg (DropdownType.NavItemPicked item), _ ) ->
            let
                ...
                -- TODO: Rename urlFromString to reflect new implementation
                actionCmd =
                    case item of
                        DropdownType.Logout ->
                            Cmd.batch [ Ports.logout (), Route.modifyUrl Route.Home ]
...
initChannels session =
    case session.player of
        ...
        -- TODO: Subscribe to channels if a user is logged in
        Nothing ->
            []
    ...
    let
        ...
        withBlur =
            case model.openDropdown of
                ...
                _ ->
                    [ Sub.map HeaderMsg (Mouse.clicks (always DropdownType.Blur)) ]
...
pageSubscriptions page model =
    ...
    case page of
        ...
        Page.ForgotPassword subModel ->
            Sub.map ForgotPasswordMsg <| ForgotPassword.subscriptions subModel
