...
import Html.Attributes as Attributes exposing (class, classList, href, placeholder, style)
...
import Ports exposing (logout, triggerFBInviteRequest)
import Route
...
type alias Rooms =
    { rooms : RoomInfoList, page : Int, totalPages : Int }


type alias RoomInfo =
    { room : String, playerCount : Int, isOwner : Bool, status : RoomStatus }


type alias RoomInfoList =
    List RoomInfo


type alias PlayerPage =
    { players : List String, page : Int, totalPages : Int }


type alias NewGame =
    { title : String, owner : String, invitees : List String }
    ...
    let
        ...
        push =
            Push.init ("private_rooms:" ++ playerName) "get_page"
                |> Push.withPayload
                    (Encode.object
                        [ ( "for", Encode.string getPageTypeParam )
...
view session model =
    div [ class "profile-container" ]
        ...
        , if model.openModal == DeleteModal then
            Modal.bottomModalView <| deleteProfileModalConfig model
          else
            text ""
        ]
    ...
    let
        ...
        updateMessages =
            case model.updateMessages of
                ...
                _ ->
                    List.map (\msg -> UpdateMessage msg) model.updateMessages
...
viewRoom listingType roomInfo =
    let
        linkAttrs =
            case listingType of
                Ongoing ->
                    [ Route.href (Route.Room "private" roomInfo.room) ]
...
maybeViewJoinDeclineBtns listingType roomInfo =
    case listingType of
        ...
        Invite ->
            ...
            , p [ class "room-btn-container" ]
                [ a
                    [ class "btn red white-text waves-effect waves-light invite-btn"
    ...
    let
        ...
        pagination =
            case model.startGameSubTab of
                ...
                PlayerSearchList ->
                    paginate (paginationObjectForSearch model) paginationConfigSearch
    in
    div [ class "player-list-container col s12" ]
...
viewSubTabs model =
    div [ class "row" ]
        [ div [ class "col s6 offset-s3" ]
            [ a
                [ class "waves-effect waves-teal btn-flat sub-tab"
...
update msg model =
    case msg of
        ...
        UpdateCurrentRooms payload ->
            handleUpdateCurrentRooms model payload

        UpdateSearchList payload ->
            handleUpdateSearchList model payload

        AcceptInvitation toRoom ->
            handleAcceptInvitation model toRoom

        DeclineInvitation fromRoom ->
            handleDeclineInvitation model fromRoom
        ...
        SubmitBlurbUpdate ->
            handleSubmitUpdate model Blurb

        HeaderClicked attribute ->
            handleHeaderClicked model attribute

        ChangeSubTab newSubTab ->
            handleChangeSubTab model newSubTab
        ...
        DeleteRoom title ->
            handleDeleteRoom model title

        LeaveRoom title ->
            handleLeaveRoom model title

        CloseModal ->
            handleCloseModal model

        Paginate type_ page_num ->
            handlePaginate model type_ page_num

        PaginateSearch page_num ->
        ...
        DeleteFailed payload ->
            handleDeleteFailed model payload
        ...
        AddInvitee invitee ->
            handleAddInvitee model invitee

        NewUpdateMessage message ->
            handleNewUpdateMessage model message

        NewErrorMessage message ->
            handleNewErrorMessage model message
...
handleHeaderClicked model attribute =
    let
        activeAttribute =
            if model.activeAttribute == attribute then
                None
            else
                attribute
...
handleSubmitUpdate model attribute =
    let
        cmd =
            case attribute of
                Email ->
                    updatePlayerPush model Email model.profile.email

                Blurb ->
                    updatePlayerPush model Blurb model.profile.blurb
...
handleNewUpdateMessage model payload =
    case Decode.decodeValue (Decode.at [ "message" ] Decode.string) payload of
        Ok message ->
            ( ( { model
                    | updateMessages = model.updateMessages ++ [ message ]
                    , activeAttribute = None
                }
              , Cmd.none
              )
...
handleClearUpdateMessage model =
    case List.tail model.updateMessages of
        Just newList ->
            ( ( { model | updateMessages = newList }, Cmd.none ), NoOp )

        Nothing ->
            ( ( { model | updateMessages = [] }, Cmd.none ), NoOp )
...
handleClearErrorMessage model =
    case List.tail model.errorMessages of
        Just newList ->
            ( ( { model | errorMessages = newList }, Cmd.none ), NoOp )

        Nothing ->
            ( ( { model | errorMessages = [] }, Cmd.none ), NoOp )
...
handleSubmitCreateGameForm model =
    let
        ...
        errorMsgs =
            case isInvalid of
                ( True, True ) ->
                    [ emptyTitleErrorMsg, noInviteesErrorMsg ]
        ...
        newModel =
            case isInvalid of
                ( True, True ) ->
                    { model | errorMessages = errorMsgs ++ model.errorMessages }
    ...
    let
        ( subTab, cmd ) =
            case String.isEmpty model.searchQuery of
                True ->
                    ( model.startGameSubTab, Cmd.none )

                False ->
                    ( PlayerSearchList, submitSearchPush model )
    in
...
handleAddInvitee model name =
    let
        ...
        newErrorMessages =
            case List.length game.invitees >= 6 of
                True ->
                    "You can only invite up to 6 players" :: model.errorMessages
    ...
    let
        ...
        clearMessages =
            case model.updateMessages of
                ...
                _ ->
                    Time.every 3000 ClearUpdateMessage
        ...
        clearErrors =
            case model.errorMessages of
                ...
                _ ->
                    Time.every 3000 ClearErrorMessage
    in
...
encodeNewGame newGame =
    Encode.object
        [ ( "title", Encode.string newGame.title )
...
paginationConfig roomListing =
    case roomListing of
        OngoingGames ->
            { onClickMsg = Paginate "current_games", linksToShow = 5 }

        Invites ->
            { onClickMsg = Paginate "invites", linksToShow = 5 }

        Players ->
            { onClickMsg = Paginate "players", linksToShow = 5 }
