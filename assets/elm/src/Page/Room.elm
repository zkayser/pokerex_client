...
update msg model =
    case msg of
        ...
        JoinFailed value ->
            handleJoinFailed model value

        SetJoinValue amount ->
            handleSetJoinValue model amount
        ...
        ActionPressed ->
            ( ( { model | modalRendered = BottomModalOpen Actions }, Cmd.none ), NoOp )

        ActionMsg action val ->
            handleActionMsg model action val
        ...
        MobileToolbarPressed ->
            ( ( { model | modalRendered = BottomModalOpen MobileMenu }, Cmd.none ), NoOp )

        ChatPressed ->
            ( ( { model | modalRendered = BottomModalOpen ModalState.Chat }, Cmd.none ), NoOp )

        CloseRaiseModal ->
            ( ( { model | modalRendered = BottomModalOpen Actions }, Cmd.none ), NoOp )

        IncreaseRaise amount ->
            handleIncreaseRaise model amount

        DecreaseRaise amount ->
            handleDecreaseRaise model amount

        SetRaise amount ->
            handleSetRaise model amount

        SetAddAmount amount ->
            handleSetAddAmount model amount
        ...
        OpenRaisePressed ->
            ( ( { model | modalRendered = RaiseModalOpen }, Cmd.none ), NoOp )
...
subscriptions model session =
    let
        ...
        withBlur =
            case model.modalRendered of
                WinningHandModal _ ->
                    Time.every 5000 ClearWinningHandModal
        ...
        withClearRoomMessage =
            case model.roomMessages of
                ...
                _ ->
                    Time.every 3000 ClearRoomMessage
    in
