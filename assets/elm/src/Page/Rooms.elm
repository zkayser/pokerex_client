...
type alias RoomInfo =
    { room : String, playerCount : Int, status : RoomStatus }
        ...
        newRooms =
            case Decode.decodeValue roomInfoDecoder payload of
                Ok newRoomInfo ->
                    List.map
                        (\info ->
                            if info.room == newRoomInfo.room then
                                newRoomInfo
                            else
                                info
                        )
        ...
        pushMessage =
            case text of
                "keyboard_arrow_right" ->
                    if onLastPage model then
                        Cmd.none
                    else
                        getPage (toString <| model.page + 1) model.socketUrl

                "keyboard_arrow_left" ->
                    if onFirstPage model then
                        Cmd.none
                    else
                        getPage (toString <| model.page - 1) model.socketUrl

                "First" ->
                    getPage "1" model.socketUrl

                "Last" ->
                    getPage (toString model.totalPages) model.socketUrl

                _ ->
                    getPage text model.socketUrl
    in
