    ...
    let
        ( newModel, cmd ) =
            case String.length model.newPassword >= 6 of
                True ->
                    ( model, Http.send ServerResult (Request.resetPassword model) )

                False ->
                    ( { model | errors = "Password must be at least 6 characters" :: model.errors }, Cmd.none )
    in
    ( ( newModel, cmd ), NoOp )
...
handleClearMessage model =
    case List.tail model.messages of
        Just newList ->
            ( ( { model | messages = newList }, Cmd.none ), NoOp )

        Nothing ->
            ( ( { model | messages = [] }, Cmd.none ), NoOp )
...
handleClearError model =
    case List.tail model.errors of
        Just newList ->
            ( ( { model | errors = newList }, Cmd.none ), NoOp )

        Nothing ->
            ( ( { model | errors = [] }, Cmd.none ), NoOp )
