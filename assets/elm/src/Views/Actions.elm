...
canCallAction actionsModel action =
    if actionsModel.isActive == False then
        ...
        case action of
            Call ->
                actionsModel.paidInRound < actionsModel.toCall

            Check ->
                actionsModel.paidInRound == actionsModel.toCall

            Raise ->
                actionsModel.chips > actionsModel.toCall
...
getAttributesFor actionsModel action =
    case action of
        ...
        Check ->
            ( "green accent-3", actionMsgWith actionsModel "action_check", "Check" )

        Fold ->
            ( "teal darken-4", actionMsgWith actionsModel "action_fold", "Fold" )
