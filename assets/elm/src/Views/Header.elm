...
import Html.Events exposing (Options, onClick)
...
viewNavBarLinks msg session page =
    case session.player of
        Just player ->
            ...
            , navBarLink (page == Helpers.Profile)
                (Route.Profile <| Player.usernameToString player.username)
                [ text "Profile" ]
...
navLinks session =
    case session.player of
        Just player ->
            [ DropdownType.Logout, DropdownType.Profile, DropdownType.Rooms ]

        Nothing ->
            [ DropdownType.Login, DropdownType.Register ]
